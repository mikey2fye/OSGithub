    ...
    */
    ...
    while (current != NULL) 
    {
        ...
        if (blk->pid == pid) 
        {
            ...
            // Add the block back to the free list based on FIFO
            list_add_to_back(freelist, blk);
            break; // Exit the loop after deallocation
        }
